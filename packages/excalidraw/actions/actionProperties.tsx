...
export const actionChangeOpacity = register({
  ...
  // The perform function updates the elements' opacity and appState
  perform: (
    elements: readonly ExcalidrawElement[],
    appState: AppState,
    value: number
    ...
    const updatedElements = changeProperty(
      ...
      true // Ensure deep copy or specific behavior
    );
  ...
  // The PanelComponent defines the UI for the opacity slider
  PanelComponent: ({
    elements,
    appState,
    updateData,
    ...
    // Helper to calculate percentage from pointer position
    const calculatePercentage = (
      ...
      const percentage = Math.min(
        100,
        Math.max(0, ((clientX - left) / width) * 100)
      );
      return Math.round(percentage);
    };

    // Drag handling for both mouse and touch events
    const handleDrag = (
      clientX: number,
      track: HTMLDivElement
    ): void => {
    ...
    const startDragging = (event: React.MouseEvent | React.TouchEvent): void => {
      const track = event.currentTarget.closest<HTMLDivElement>(".custom-slider");
      if (!track) return;

      // Determine the clientX based on event type
      const clientX =
        "touches" in event ? event.touches[0].clientX : event.clientX;
      handleDrag(clientX, track);
      ...
      <label className="control-label">
        ...
        <div
          ...
          style={{
            ...
            height: "10px",
            background:
              "linear-gradient(to left, var(--link-color), var(--input-bg-color))",
            borderRadius: "5px",
            cursor: "pointer",
          }}
        ...
        >
          <div
            ...
            style={{
              ...
              width: "16px",
              height: "16px",
              backgroundColor: "var(--button-destructive-bg-color)",
